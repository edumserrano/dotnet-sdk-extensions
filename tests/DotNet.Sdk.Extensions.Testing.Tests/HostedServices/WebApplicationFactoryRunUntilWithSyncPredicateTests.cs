...
namespace DotNet.Sdk.Extensions.Testing.Tests.HostedServices
{
    ...
    /// </summary>
    public class WebApplicationFactoryRunUntilWithSyncPredicateTests
    {
        public static TheoryData<HostedServicesWebApplicationFactory, RunUntilPredicate, Type, string> SyncPredicateValidateArgumentsData =>
            ...
            string exceptionMessage)
        {
            var exception = Should.Throw(
                actual: () => RunUntilExtensions.RunUntilAsync(webApplicationFactory, predicate),
                exceptionType: exceptionType);
            exception.Message.ShouldBe(exceptionMessage);
        }

        public static TheoryData<HostedServicesWebApplicationFactory, RunUntilPredicate, Action<RunUntilOptions>, Type, string> SyncPredicateWithOptionsValidateArgumentsData =>
        ...
        {
            var exception = Should.Throw(
                actual: () => RunUntilExtensions.RunUntilAsync(webApplicationFactory, predicate, configureOptions),
                exceptionType: exceptionType);
            exception.Message.ShouldBe(exceptionMessage);
        }
        ...
        public async Task RunUntilSyncPredicateTimeoutOption()
        {
            ...
            var runUntilTask = webApplicationFactory.RunUntilAsync(() => callCount == 4, options => options.Timeout = TimeSpan.FromMilliseconds(100));
