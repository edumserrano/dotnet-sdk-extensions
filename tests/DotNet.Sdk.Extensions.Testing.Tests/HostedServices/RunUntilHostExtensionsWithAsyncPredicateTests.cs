...
namespace DotNet.Sdk.Extensions.Testing.Tests.HostedServices
{
    ...
    public class RunUntilHostExtensionsWithAsyncPredicateTests
    {
        ...
        /// and the predicate to stop the Host after receiving at least 3 calls then the timeout should be triggered before the predicate is met.
        /// </summary>
        [Fact]
        public async Task TimeoutOption()
        {
            ...
            var runUntilTask = host.RunUntilAsync(() => Task.FromResult(callCount >= 3), options => options.Timeout = TimeSpan.FromSeconds(1));
