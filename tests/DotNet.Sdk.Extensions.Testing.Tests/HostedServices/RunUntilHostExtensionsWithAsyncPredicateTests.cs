...
#pragma warning disable CA2000 // Dispose objects before losing scope - the test method will do the dispose
    ...
    public async Task TimeoutOption()
    {
        ...
        var runUntilTask = host.RunUntilAsync(() => Task.FromResult(callCount >= 4), options => options.Timeout = TimeSpan.FromSeconds(2));
    ...
    /// so that the timeout occurs even before the first predicate check is made.
    /// </summary>
    [Fact]
