        ...
        app
            ...
            .UseEndpoints(endpoints =>
            {
                endpoints.MapGet("/basic-client", async context =>
                {
                    ...
                    await context.Response.WriteAsync($"Basic http client returned: {response.IsSuccessStatusCode}", cancellationToken: context.RequestAborted);
                });
                endpoints.MapGet("/named-client", async context =>
                {
                    ...
                    await context.Response.WriteAsync($"Named http client (my-named-client) returned: {response.IsSuccessStatusCode}", cancellationToken: context.RequestAborted);
                });
                endpoints.MapGet("/typed-client", async context =>
                {
                    ...
                    await context.Response.WriteAsync($"MyApiClient typed http client returned: {response}", cancellationToken: context.RequestAborted);
                });
                endpoints.MapGet("/typed-client-with-custom-name", async context =>
                {
                    ...
                    await context.Response.WriteAsync($"MyApiClient typed http client with custom name my-typed-client returned: {response}", cancellationToken: context.RequestAborted);
                });
                endpoints.MapGet("/typed-client-with-custom-name-2", async context =>
                {
                    ...
                    var typedClientWithCustomName = typedHttpClientFactory.CreateClient(namedHttpClient);
                    var response = await typedClientWithCustomName.DoSomeHttpCall();
                    await context.Response.WriteAsync($"MyApiClient typed http client with custom name my-typed-client-2 returned: {response}", cancellationToken: context.RequestAborted);
                });
            });
    }
}
